...
export const deleteUserFromRequest = async (req, res) => {
    try {
        ...
        // Check if the logged-in user is a follower of the request sender
        const isRequestSend = loggedInUser.followers.includes(requestSenderId);
        ...
        console.error('Error in deleteUserFromRequest:', error);
        return response(res, 500, 'Internal server error', error.message);
    }
};
...
//get all frined request fro user
export const getAllUserForRequest = async (req, res) => {
    try {
        const loggedInUserId = req.user.userId;
